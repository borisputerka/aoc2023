...
gear ratio is 16345. The second gear is in the lower right; its gear ratio is 451490. (The * adjacent to 617 is not
a gear because it is only adjacent to one part number.) Adding up all of the gear ratios produces 467835.
...
func main() {
	...
	rAsterisk := regexp.MustCompile("\\*")
	var field []string
	line := 0
	positionsWithNumbers := make(map[string]int)
	var symbolsPositions, asteriskPositions [][]int
	for sc.Scan() {
		field = append(field, sc.Text())
		numbersInLine := rN.FindAllString(sc.Text(), -1)
		symbolsPositionInLine := rS.FindAllStringIndex(sc.Text(), -1)
		asteriskPositionInLine := rAsterisk.FindAllStringIndex(sc.Text(), -1)
		numbersPositionInline := rN.FindAllStringIndex(sc.Text(), -1)
		for i, n := range numbersPositionInline {
			number, _ := strconv.Atoi(numbersInLine[i])
			positionsWithNumbers[strings.Trim(strings.Trim(fmt.Sprint(n), "]"), "[")+" "+strconv.Itoa(line)] = number
		}

		for _, s := range symbolsPositionInLine {
			s = append(s[0:1], line)
			symbolsPositions = append(symbolsPositions, s)
		}

		for _, a := range asteriskPositionInLine {
			a = append(a[0:1], line)
			asteriskPositions = append(asteriskPositions, a)
		}
	...
	visitedGearPositions := make(map[string]bool)
	for k, v := range positionsWithNumbers {
		numberLine, _ := strconv.Atoi(strings.Split(k, " ")[2])
		numberFrom, _ := strconv.Atoi(strings.Split(k, " ")[0])
		numberTo, _ := strconv.Atoi(strings.Split(k, " ")[1])
		numberTo = numberTo - 1

		// Part 1
		for _, s := range symbolsPositions {
			symbolLine := s[1]
			symbolPosition := s[0]
			part1Done := false
			if numberLine-1 == symbolLine || numberLine == symbolLine || numberLine+1 == symbolLine {
				if symbolPosition >= numberFrom-1 && symbolPosition <= numberTo+1 && !part1Done {
					part1Result += v
					part1Done = true
				}
			}
		...
		// Part 2
		if visitedGearPositions[k] {
			continue
		}
		for _, a := range asteriskPositions {
			asteriskLine := a[1]
			asteriskPosition := a[0]
			if numberLine-1 == asteriskLine || numberLine == asteriskLine || numberLine+1 == asteriskLine {
				if asteriskPosition >= numberFrom-1 && asteriskPosition <= numberTo+1 {
					for kk, vv := range positionsWithNumbers {
						if vv != v {
							nNumberLine, _ := strconv.Atoi(strings.Split(kk, " ")[2])
							nNumberFrom, _ := strconv.Atoi(strings.Split(kk, " ")[0])
							nNumberTo, _ := strconv.Atoi(strings.Split(kk, " ")[1])
							nNumberTo = nNumberTo - 1
							if nNumberLine-1 == asteriskLine || nNumberLine == asteriskLine || nNumberLine+1 == asteriskLine {
								if asteriskPosition >= nNumberFrom-1 && asteriskPosition <= nNumberTo+1 {
									visitedGearPositions[kk] = true
									part2Result += v * vv
									break
								}
							}
