...
after all! I bet the gardener would know. He's on a different island, though - er, the small kind surrounded by water,
...
of winning numbers. The first match makes the card worth one point and each match after the first doubles the point value of that card.
...
that the original card 10 won: cards 11, 12, 13, 14, and 15. This process repeats until none of the copies cause
...
Process all of the original and copied scratchcards until no more scratchcards are won. Including the original
...
func main() {
	...
	totalNTGameTickets := make(map[int]int)
	var part1Result float64
	var part2Result int
	game := 1
	for sc.Scan() {
		totalNTGameTickets[game] += 1
		ticketNumbers := strings.Split(strings.Split(sc.Text(), ": ")[1], "|")
		winningNumbers := stringSliceToIntSlice(r.FindAllString(ticketNumbers[0], -1))
		myNumbers := stringSliceToIntSlice(r.FindAllString(ticketNumbers[1], -1))
		winningNCount := getCountOfWinningNumbers(winningNumbers, myNumbers)
		if winningNCount > 0 {
			part1Result += math.Pow(float64(2), float64(winningNCount-1))
			for i := 1; i <= winningNCount; i++ {
				totalNTGameTickets[game+i] += totalNTGameTickets[game]
			}
		}
	...
	for k, v := range totalNTGameTickets {
		// We only count real games played (map stores also some extra because of line 32)
		if k <= game {
			part2Result += v
		}
	}
...
func getCountOfWinningNumbers(s1, s2 []int) int {
