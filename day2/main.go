...
The power of the minimum set of cubes in game 1 is 48. In games 2-5 it was 12, 1560, 630, and 36, respectively. Adding
up these five powers produces the sum 2286.

For each game, find the minimum set of cubes that must have been present. What is the sum of the power of these sets?
*/

var part1PossibleGamesColorCounts = map[string]int{
	"red":   12,
	"green": 13,
	"blue":  14,
}
...
func main() {
	...
	var resultPart1, resultPart2, allGamesSum, impossibleGamesSum int
	lineCount := 0
	for sc.Scan() {
		lineCount++
		allGamesSum += lineCount
		parsedInputLine := strings.Split(strings.Replace(strings.Split(sc.Text(), ": ")[1], ";", ",", -1), ", ")
		part1PossibleGame := true
		part2MinimumGamesColorCounts := map[string]int{
			"red":   0,
			"green": 0,
			"blue":  0,
		}

		for _, s := range parsedInputLine {
			colorCount, _ := strconv.Atoi(strings.Split(s, " ")[0])
			color := strings.Split(s, " ")[1]
			if part1PossibleGamesColorCounts[color] < colorCount && part1PossibleGame {
				impossibleGamesSum += lineCount
				part1PossibleGame = false
			}

			if part2MinimumGamesColorCounts[color] < colorCount {
				part2MinimumGamesColorCounts[color] = colorCount
			}
		}

		gamePowers := 1
		for _, v := range part2MinimumGamesColorCounts {
			gamePowers *= v
		}

		resultPart2 += gamePowers
	}

	resultPart1 = allGamesSum - impossibleGamesSum
